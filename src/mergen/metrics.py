...
def quality_report(
    ...
    if mc_samples > 0:
        print(f"  {_YELLOW}[METRICS]{_RESET}  "
              f"Computing MC baseline ({mc_samples} random designs)...",
              end='\r')
        ...
        print(f"  {_GREEN}[METRICS]{_RESET}  "
              f"MC baseline complete ({mc_samples} random designs).     ")
...
def _fmt_pct(p) -> str:
    ...
    return f'{colour}{p:.0f}th pct{_RESET}'
...
def _print_table(
    ...
    if has_mc:
        hdr = f"  {'Metric':<26} {'Value':>10}  {'Baseline':>10}  {'Rank':>10}  Direction"
    else:
        hdr = f"  {'Metric':<26} {'Value':>10}  Direction"
    print(hdr)
    print(sep)
    ...
    for m in metric_names:
        ...
        if has_mc and m in mc_vals:
            ...
            print(f"  {label:<26} {_fmt_val(val):>10}  "
                  f"{_fmt_val(med):>10}  {_fmt_pct(pct):>22}  {direc}")
        else:
            print(f"  {label:<26} {_fmt_val(val):>10}  {direc}")
    ...
    if has_mc:
        print(sep)
        print(f"  Baseline: {len(mc_vals[next(iter(mc_vals))])} random designs "
              f"from feasible space  |  Rank = percentile among random designs")
    print('═' * W)
    print()
