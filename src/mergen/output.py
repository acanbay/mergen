...
def plot_quality(result, title: bool = True,
    ...
    # Baseline markers
    # NOTE: axvline's ymin/ymax are in AXES coordinates [0, 1] and are
    # NOT affected by ax.invert_yaxis() below. The bars are drawn in data
    # coordinates so invert_yaxis flips them visually, but axes-coord
    # markers stay put. To align baseline markers with their bars after
    # invert, we mirror the y-range: row i (data y=i) is displayed at
    # axes y position (N - i - 1) / N after invert.
    N = len(metric_names)
    for i, (bl, v) in enumerate(zip(baselines, values)):
        if bl is not None and not np.isnan(bl):
            row_top    = (N - i)     / N    # top of row in axes coords
            row_bottom = (N - i - 1) / N    # bottom of row in axes coords
            # Span 80% of the row height, centred
            mid    = (row_top + row_bottom) / 2
            half_h = 0.4 / N
            ax.axvline(x=bl, ymin=mid - half_h, ymax=mid + half_h,
